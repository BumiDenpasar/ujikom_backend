                ...
                <div class="flex flex-col items-center p-6 text-gray-900">
                    ...
                    <di class="flex flex-wrap justify-center w-full">
                        @foreach ($categories as $category)
                            <div class="relative w-56 m-2 overflow-hidden transition-all duration-500 bg-center bg-cover drop-shadow-lg hover:drop-shadow-2xl h-80 rounded-2xl group hover:scale-110"
                                style="background-image: url({{ $category->img }})">
                                <h1
                                    class="z-10 flex items-center justify-center w-full h-full text-white duration-500 bg-black title bg-opacity-30 backdrop-blur-sm group-hover:backdrop-blur-none">
                                    {{ $category->title }}</h1>

                                <div class="hidden group-hover:block">
                                    <button onclick="edit_{{ $category->id }}.showModal()"
                                    ...
                                    <button onclick="delete{{ $category->id }}.showModal()"
